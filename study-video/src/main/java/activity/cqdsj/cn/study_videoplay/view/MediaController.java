...
public class MediaController extends FrameLayout {
    ...
    private void initControllerView(View v) {
        mPauseButton = (RelativeLayout) v.findViewById(R.id.mediacontroller_play_pause);
        ivStart = (ImageView) v.findViewById(R.id.mc_play_on);
        ivPause = (ImageView) v.findViewById(R.id.mc_play_pause);
        if (mPauseButton != null) {
            mPauseButton.requestFocus();
            mPauseButton.setOnClickListener(mPauseListener);
        }

        mPausePP = (ImageView) v.findViewById(R.id.mediacontroller_play_pp);
    ...
    public void show(int timeout) {
        ...
        if (timeout != 0) {
            //清空队列的中的隐藏MediaController的消息
            mHandler.removeMessages(FADE_OUT);
            //3s之后隐藏MediaController
    ...
    public void setGoIcon() {
  	  mProgress.setThumb(getResources().getDrawable(R.drawable.ic_contact_us));
    }
...
//                DebugLog.d(TAG, "MediaController already removed");
    ...
    private void updatePausePlay() {
        if (mRoot == null || mPauseButton == null)
            return;
        myFlag=MyVideoView.myFlag;

        if (myFlag==1) {
        	if (mPausePP.getVisibility() == View.INVISIBLE) {
        		mPausePP.setImageResource(getResources().getIdentifier("video_player_speed", "drawable", mContext.getPackageName()));
        		mPausePP.setVisibility(View.VISIBLE);
    		...
    		if (mPausePP.getVisibility() == View.INVISIBLE) {
    			mPausePP.setImageResource(getResources().getIdentifier("video_player_quitdc", "drawable", mContext.getPackageName()));
        		mPausePP.setVisibility(View.VISIBLE);
    		...
    		if (mPausePP.getVisibility() == View.VISIBLE) {
        		mPausePP.setVisibility(View.INVISIBLE);
        		myFlag = 0;
    		}
...
//            System.out.println("当前进度:"+progress);
        ...
        public void onStopTrackingTouch(SeekBar bar) {
            /*final long position = (mDuration * bar.getProgress()) / 1000 ;
            mPlayer.seekTo((mDuration * bar.getProgress()) / 1000);*/
            System.out.println("当前进度："+bar.getProgress());
            if (!mInstantSeeking)
                mPlayer.seekTo((mDuration * bar.getProgress()) / 1000);
            if (mInfoView != null) {
                mInfoView.setText("");
            ...
            mHandler.sendEmptyMessageDelayed(SHOW_PROGRESS, 0);
        }
    };
