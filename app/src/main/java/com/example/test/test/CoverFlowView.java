...
public class CoverFlowView<T extends CoverFlowAdapter> extends View {
    public static final String TAG = "CoverFlowView";
    ...
    private static final float MOVE_POS_MULTIPLE = 3.0f;
    ...
    /**
     * <ul>
     * <li>Record origin width and height of images</li>
     * <li>记录图片初始的宽度和高度</li>
     * </ul>
     */
    ...
    private void initAttributes(Context context, AttributeSet attrs) {
        ...
        VISIBLE_VIEWS = totalVisibleChildren >> 1;//相当于num除以2
        Log.e(TAG, "initAttributes: " + totalVisibleChildren + "--" + VISIBLE_VIEWS);
    ...
    protected void onDraw(Canvas canvas) {
        ...
        int i = 0;
        int mid = (int) Math.floor(offset + 0.5);//0
        //3%2=1; 3/2=1;
        int rightChild = (mVisibleChildCount % 2 == 0) ? (mVisibleChildCount >> 1) - 1
                : mVisibleChildCount >> 1;
        int leftChild = mVisibleChildCount >> 1;//1

        // draw the left children
        int startPos = mid - leftChild;//-1
        Log.e(TAG, "onDraw: rightChild=" + rightChild + ", leftChild=" + leftChild + ", startPos=" + startPos + ", mid=" + mid);
        for (i = startPos; i < mid; ++i) {//-1
            drawChild(canvas, i, i - offset);
        }

        // draw the right children
        int endPos = mid + rightChild;//1
        for (i = endPos; i >= mid; --i) {//1,0
            drawChild(canvas, i, i - offset);
        }
    ...
    protected final void drawChild(Canvas canvas, int position, float offset) {
        Log.e(TAG, "drawChild: position=" + position + ", offset=" + offset);
        int actuallyPosition = getActuallyPosition(position);//-1--0,1--2,0--1
        Log.e(TAG, "drawChild: actuallyPosition= " + position + "--" + actuallyPosition);
        final Bitmap child = mAdapter.getImage(actuallyPosition);
        //final Bitmap reflection = obtainReflection(actuallyPosition, child);

        int[] wAndh = mImageRecorder.get(actuallyPosition);//0,2,1
        if (wAndh == null) {
    ...
    private void makeChildTransfromer(Bitmap child, int position, float offset) {
        Log.e(TAG, "makeChildTransfromer: offset="+offset );
...
//        Log.e(TAG, "makeChildTransfromer: alpha="+alpha );
...
//                child, position, offset);
     ...
     * </ul>
     *
     * @param transfromer    childTransfromMatrix
    ...
    private int getActuallyPosition(int position) {
        ...
        position += VISIBLE_VIEWS;//2
        //Log.e(TAG, "getActuallyPosition: position="+VISIBLE_VIEWS+"--"+position );
